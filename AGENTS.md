...
- 활성 단계는 `P3-01`이며, `P2-01`(F-007·F-024·F-013 독립 HWPX 템플릿의 Kordoc `fill -j` 치환)은 라벨 기반 재구축으로 `fill -j → validate → render → 재파싱` 왕복을 통과해 기술 검증 범위 `DONE`으로 전환됨. 다만 한컴 수동 열기·PDF 비교(`P2-02`)는 아직 `TODO`이므로, HWPX 서식을 실제 반영·배포하는 작업은 P2-02가 통과한 뒤에만 수행함. 최신 상태는 자주 바뀌므로 이 파일에 세부를 하드코딩하지 않고 [task.md](task.md)·[로그.md](로그.md)를 직접 확인함.
